...
class PlaidService:
    ...
    def __init__(self):
        ...
        configuration = plaid.Configuration(
            host=plaid.Environment.Sandbox,  # <--- Already correctly set to Sandbox
            api_key={
        ...
        # --- 2. ADD SANDBOX FLAG ---
        self.is_sandbox = (configuration.host == plaid.Environment.Sandbox)
        # ---------------------------

    # --- 3. ADD NEW SANDBOX-ONLY METHOD ---
    def create_sandbox_access_token_for_user(self, user, institution_id: str = 'ins_109508') -> Dict:
        """
        [DEVELOPMENT ONLY]
        Creates a sandbox public token, exchanges it for an access token,
        and saves it directly to the user's profile.
        This bypasses the need for the Plaid Link front-end UI.
        
        Args:
            user: Django User object
            institution_id: The Plaid sandbox institution ID to use.
        
        Returns:
            Dictionary with access_token or error.
        """
        if not self.is_sandbox:
            return {'success': False, 'error': 'This function is only for sandbox environment.'}
        ...
        try:
            ...
            print(f"SANDBOX: Successfully created and saved access_token for user {user.id}")
            ...
            print(f"Error creating sandbox access token: {e}")
            return {'success': False, 'error': str(e)}
        except Exception as e:
            print(f"Error saving sandbox access token: {e}")
            return {'success': False, 'error': str(e)}
    # --------------------------------------
    ...
    def create_link_token(self, user) -> str:
        ...
        (This is for the REAL front-end flow)
    ...
    def sync_transactions(self, user, force_full_sync: bool = False) -> Dict:
        ...
        try:
            ...
            # --- 4. ADD SANDBOX DELAY ---
            # Wait for Plaid to prepare transaction data in Sandbox
            if self.is_sandbox:
                print("SANDBOX: Waiting 10 seconds for transactions to generate...")
                time.sleep(2)
            # --------------------------
            
            # Determine date range
            end_date = datetime.now().date()
            print("Debug: Determining start date for transaction sync...")
            ...
            print("Debug: Fetching transactions from Plaid...")
            ...
            print("Debug: Processing and saving transactions...")
    ...
    def _process_transactions(self, user, transactions: List[Dict]) -> Dict:
        ...
        for txn in transactions:
            ...
            date_val = txn.get('date')  # Get the raw date value
            amount_val = txn.get('amount')

            # --- VALIDATION ---
            # Skip transaction if critical data is missing
            if date_val is None or amount_val is None:
                print(f"Skipping transaction, missing date or amount: {txn.get('name')}")
                continue

            # --- FIX: Handle date object OR date string ---
            transaction_date = None
            if isinstance(date_val, date):
                # It's already a date object, just use it
                ...
                try:
                    ...
                    print(f"Skipping transaction, invalid date format: {date_val}")
                    continue
            else:
                # It's some other unexpected type
                print(f"Skipping transaction, unknown date type: {type(date_val)}")
                continue
            # ------------------------------------------

            # --- ROBUST DESCRIPTION ---
            description = txn.get('merchant_name') or txn.get('name', 'Unknown')
            
            defaults = {
                'description': description,
                'amount': float(amount_val),
                'date': transaction_date,  # <-- Use the cleaned date
                'category': category,
            }
    ...
    def _get_or_create_category(self, transaction: Dict) -> Category:
        ...
        # if 'personal_finance_category' in transaction and transaction['personal_finance_category']:
        ...
        if transaction['personal_finance_category']:
            category_name = transaction['personal_finance_category']['primary']
        print("Debug: Mapped Plaid category:", plaid_category, "to", category_name)
        # Get or create category
